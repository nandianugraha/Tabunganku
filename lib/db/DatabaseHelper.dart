...
class DatabaseHelper {
  static final _databaseName = "history_pengeluaran.db";
  static final _databaseVersion = 1;

  static final table = 'pengeluaran_harian';
  ...
  Future<int> insert(History history) async {
    ...
    return await db.insert(table, {'productName': history.productName, 'price': history.price, 'date': history.date});
  }
  ...
  Future<int> update(History history) async {
    Database db = await instance.database;
    int id = history.toMap()['id'];
    return await db.update(table, history.toMap(), where: '$columnId = ?', whereArgs: [id]);
  }
